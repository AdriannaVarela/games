// Seleccionar el contenedor donde se mostrarán los juegos favoritos
const container = document.getElementById("favoritosGrid");
console.log("Contenedor 'favoritosGrid' obtenido:", container);

// 1. Obtener el userId almacenado en localStorage (guardado tras el login)
const userId = localStorage.getItem("userId");
console.log("userId obtenido desde localStorage:", userId);
...
// Función para obtener los favoritos del usuario desde la API en el backend (MongoDB)
async function fetchFavoritosUsuario(userId) {
  console.log("Llamando a fetchFavoritosUsuario con userId:", userId);
  try {
    // Realizamos una petición GET a la API para obtener el objeto usuario
    const res = await axios.get(`/api/users/${userId}`);
    console.log("Respuesta de la API al obtener el usuario:", res.data);
    // Se espera que el objeto usuario tenga el campo "favoritos" que es un array de IDs de juegos
...
// Función para mostrar los juegos favoritos en la interfaz
async function mostrarFavoritos() {
  ...
  // Si no existe el userId, redirigimos al login
  if (!userId) {
    console.error("No se encontró un userId en localStorage. Redirigiendo a login.");
    window.location.pathname = "/login";
    return;
  }

  // Obtener el arreglo de IDs de juegos favoritos desde la API (MongoDB)
  const favoritosIds = await fetchFavoritosUsuario(userId);
  console.log("IDs de juegos favoritos obtenidos (original):", favoritosIds);
  
  // Si no se encuentran favoritos, mostramos un mensaje adecuado
  if (!favoritosIds || favoritosIds.length === 0) {
    container.innerHTML = "<p>No tienes juegos favoritos guardados.</p>";
    return;
  }
  
  // Convertimos los favoritosIds a números
  const favoritosNumeros = favoritosIds.map(id => Number(id));
  console.log("IDs de juegos favoritos convertidos a números:", favoritosNumeros);
  
  // Filtramos el arreglo local "dataGames" que contiene los datos completos de cada juego
  const juegosFavoritos = dataGames.filter(juego => favoritosNumeros.includes(juego.id));
  console.log("Juegos favoritos encontrados en dataGames:", juegosFavoritos);

  // Si no hay coincidencias en dataGames, informamos al usuario
  if (juegosFavoritos.length === 0) {
    container.innerHTML = "<p>No se encontraron juegos favoritos en nuestra base de datos.</p>";
    return;
  }
  console.log("Tipo de favorito en DB:", typeof favoritosIds[0]);
  console.log("Tipo de juego.id en dataGames:", typeof dataGames[0].id);
  // Por cada juego favorito, creamos una tarjeta visual con la información del juego
  juegosFavoritos.forEach(juego => {
    console.log("Creando tarjeta para juego:", juego.title);
    
    const card = document.createElement("div");
    card.classList.add("producto");

    card.innerHTML = `
      <img class="producto__imagen" src="${juego.thumbnail}" alt="${juego.title}">
      <div class="producto__informacion">
        <p class="producto__nombre">${juego.title}</p>
        <p class="producto__descripcion">${juego.short_description}</p>
        <p class="producto__descripcion">Plataforma: ${juego.platform}</p>
        <p class="producto__descripcion">Género: ${juego.genre}</p>
    ...
    console.log("Tarjeta agregada al contenedor");
  });
}

// Ejecutar la función mostrarFavoritos una vez que el contenido del DOM esté completamente cargado
...
if (closeBtn) {
  console.log("Botón cerrar sesión obtenido:", closeBtn);
  
  closeBtn.addEventListener("click", async (e) => {
    console.log("Botón cerrar sesión clickeado. Evento:", e);
    try {
