...
// Ejemplo de data local (puedes reemplazarlo por tu data real o una llamada a un endpoint)
const dataGames = [ 
...
// Variable para almacenar en memoria los favoritos
let favoritos = [];
...
function mostrarJuegos(juegos) {
  ...
  juegos.forEach((juego) => {
    ...
    card.innerHTML = `
      <div class="producto">
        <img class="producto__imagen" src="${juego.thumbnail}" alt="imagen">
        <div class="productp__informacion">
          ...
          <p class="producto__descripcion">Genero: ${juego.genre}</p>
          <div class="enlace_juego">
            <a class="enlace_juego" href="${juego.game_url}" target="_blank">Jugar</a> <br>
			 
          </div>
		    <div class="enlace_juego">
           <button class="agregar-favorito" data-id="${juego.id}">Agregar a favoritos</button>
...
// Función para actualizar favoritos en la DB usando el userId obtenido de localStorage
async function updateFavoritesInDB(favoritos) {
  // Se obtiene el userId desde localStorage
  ...
  try {
    // Se envía la petición PUT a /api/users/:id con los favoritos
    const res = await axios.put(`/api/users/${userId}`, { favoritos });
    console.log("Favoritos actualizados en la DB:", res.data);
    return res.data;
  } catch (error) {
    console.error("Error al actualizar favoritos en la DB:", error);
    throw error;
  }
}

// Evento para agregar juegos a favoritos
document.addEventListener("click", async (e) => {
  if (e.target.classList.contains("agregar-favorito")) {
    const id = e.target.dataset.id;
    // Verificamos si el juego ya está en la lista de favoritos (en memoria)
    if (!favoritos.includes(id)) {
      favoritos.push(id);
      try {
        await updateFavoritesInDB(favoritos);
        alert("Agregado a favoritos y actualizado en la DB.");
      } catch (error) {
        alert("Error al actualizar la DB");
      }
    } else {
